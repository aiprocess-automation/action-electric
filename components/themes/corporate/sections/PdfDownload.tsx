  ...
  return (
    <section className="bg-slate-50 px-6 py-20">
      <div className="mx-auto max-w-4xl">
        {data.eyebrow && (
          <div className="inline-block rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[var(--color-primary)]">
            {data.eyebrow}
          </div>
        )}
        ...
        {data.description && (
          <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-600">{data.description}</p>
        )}
          ...
          {data.files.map((f) => (
            <li key={f.url}>
              <a
                ...
                className="group flex items-center gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md"
              >
